...
public class ProductoCreateService implements IProductoCreateService {
    ...
    private final ICapitalizacionServices capitalizar;
    ...
    public ProductoResponseDto create(ProductoCreateRequestDto dto) {
        ...
        dto.setNombre(capitalizar.capitalizar(dto.getNombre()));

        if (productoExistByNameService.existByName(dto.getNombre())) {
            throw new BadRequestException("El nombre del producto ya existe");
        }

        dto.setDescripcion(capitalizar.capitalizar(dto.getDescripcion()));

        

        Producto productoAGuardar = ProductoMapper.toEntity(dto, categoriaRepository.findById(dto.getCategoriaId()).orElseThrow(() -> new NotFoundException("Categoria no encontrada")));
